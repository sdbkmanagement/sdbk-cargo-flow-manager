...
DECLARE
  ...
  v_qte NUMERIC;
BEGIN
  -- Priorité à la destination officielle, sinon le lieu d'arrivée
  v_destination := COALESCE(NULLIF(trim(NEW.destination), ''), NEW.lieu_arrivee);
  ...
  -- Règle métier: le client "Station Dabola Route Kouroussa" relève de Dabola
  IF v_dest_normalized ILIKE '%dabola%' THEN
    v_dest_normalized := 'dabola';
    NEW.destination := 'Dabola';
  END IF;
