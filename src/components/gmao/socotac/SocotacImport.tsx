...
import { normaliserImmat } from './socotacUtils';
...
export const SocotacImport: React.FC<Props> = ({ open, onOpenChange, onImported }) => {
  ...
  const analyser = async (file?: File) => {
    ...
    try {
      ...
      const ws = wb.Sheets[wb.SheetNames.find((n) => n.toLowerCase().includes('socotac')) || wb.SheetNames[0]];
      ...
      setLignes(out);
      toast({ title: `${out.length} contrôle(s) détecté(s)`, description: 'Vérifiez puis lancez l’importation.' });
    } catch (e: any) {
  ...
  const importer = async () => {
    ...
    try {
      ...
      const n = await socotacService.createMany(payload);
      toast({ title: 'Importation terminée', description: `${n} contrôle(s) importé(s). Échéances et statuts recalculés.` });
      setLignes([]);
      onImported?.();
      onOpenChange(false);
    } catch (e: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Importer le fichier Socotac.xlsx</DialogTitle>
        </DialogHeader>

        <p className="text-sm text-muted-foreground">
          Les colonnes Immatriculation tracteur/remorque, conducteur, contacts, date de contrôle, observation, accepter et rejeter
          sont reprises. Les échéances, jours restants et statuts sont recalculés automatiquement par la GMAO.
        </p>
