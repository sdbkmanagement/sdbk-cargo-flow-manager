...
import { controlesAnnuelsService, ControleAnnuel, ControleAnnuelDocument } from '@/services/controlesAnnuels';
import { useToast } from '@/hooks/use-toast';
import { useAuth } from '@/contexts/AuthContext';
import { useGmao } from '../GmaoContext';
import { joursRestants, normaliserImmat } from '../socotac/socotacUtils';
import { CLASSE_STATUT_ANNUEL, LIBELLE_STATUT_ANNUEL, prochainControleAnnuel, statutAnnuel } from './annuelUtils';

interface Props {
  open: boolean;
  onOpenChange: (o: boolean) => void;
  controle?: ControleAnnuel | null;
...
export const ControleAnnuelForm: React.FC<Props> = ({ open, onOpenChange, controle, immatTracteurInitial, onSaved }) => {
  const { toast } = useToast();
  const { user } = useAuth();
  const { equipements } = useGmao();
  const [form, setForm] = useState<Record<string, string>>({ ...vide });
  const [documents, setDocuments] = useState<ControleAnnuelDocument[]>([]);
  ...
  const dateProchain = form.date_controle ? prochainControleAnnuel(form.date_controle) : null;
  const jours = joursRestants(dateProchain);
  const statut = statutAnnuel(jours);

  const televerser = async (files: FileList | null) => {
    if (!files?.length) return;
    setUpload(true);
    try {
      const ajouts: ControleAnnuelDocument[] = [];
      for (const f of Array.from(files)) ajouts.push(await controlesAnnuelsService.uploadDocument(f));
  ...
  const enregistrer = async () => {
    ...
    try {
      if (controle) {
        await controlesAnnuelsService.update(controle.id, { ...payload, updated_by: user?.id || null, updated_by_nom: nomUtilisateur });
        toast({ title: 'Contrôle annuel mis à jour' });
      } else {
        await controlesAnnuelsService.create({ ...payload, created_by: user?.id || null, created_by_nom: nomUtilisateur });
        toast({ title: 'Contrôle annuel enregistré', description: `Prochaine échéance : ${dateProchain}` });
      }
      onSaved?.();
      onOpenChange(false);
    } catch (e: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{controle ? 'Modifier le contrôle annuel' : 'Nouveau contrôle annuel'}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-5">
          ...
          <section className="space-y-3">
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Contrôle annuel</h3>
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div>
                <Label>Prochain contrôle (auto +12 mois)</Label>
              ...
              <div>
                ...
                <div className="flex h-10 items-center gap-2">
                  <Badge className={CLASSE_STATUT_ANNUEL[statut]}>{LIBELLE_STATUT_ANNUEL[statut]}</Badge>
