...
export const DriversImport: React.FC<DriversImportProps> = ({ onSuccess, onClose }) => {
  ...
  const handleImport = async () => {
    ...
    setIsImporting(true);
    try {
      const result = await chauffeursService.importChauffeurs(file);
  ...
  const handleDownloadTemplate = () => {
    const templateUrl = '/templates/template-chauffeurs.xlsx';
    const link = document.createElement('a');
    link.href = templateUrl;
    link.download = 'template-chauffeurs.xlsx';
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Importez vos chauffeurs depuis un fichier Excel
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <Button 
          ...
          className="w-full"
        >
          {isImporting ? 'Import en cours...' : 'Importer les chauffeurs'}
        </Button>
