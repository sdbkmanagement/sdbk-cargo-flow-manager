...
export const managementDashboardService = {
  async getKPIs(): Promise<ManagementKPIs> {
    ...
    return {
      ...
      chiffreAffaires: facturesMensuelles.filter(f => f.statut === 'paye' || f.statut === 'payee').reduce((s, f) => s + (f.montant_ttc || 0), 0),
      ...
      facturesPayees: facturesMensuelles.filter(f => f.statut === 'paye' || f.statut === 'payee').length,
